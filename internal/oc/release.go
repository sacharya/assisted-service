...
func (r *release) GetMCOImage(log logrus.FieldLogger, releaseImage string, releaseImageMirror string, pullSecret string) (string, error) {
	...
	if releaseImageMirror != "" {
		...
		if err != nil {
			log.Errorf("failed to get mco image from mirror release image %s", releaseImageMirror)
			return "", err
		}
	} else {
		mcoImage, err = getMCOImageFromRelease(log, releaseImage, pullSecret, false)
		if err != nil {
			log.Errorf("failed to get mco image from release image %s", releaseImage)
			return "", err
		}
	}
	return mcoImage, err
...
func getMCOImageFromRelease(log logrus.FieldLogger, releaseImage string, pullSecret string, insecure bool) (string, error) {
	...
	if err != nil {
		log.Errorf("error running \"oc adm release info\" for release %s", releaseImage)
		return "", err
	}
	return mcoImage, nil
}
...
func (r *release) Extract(log logrus.FieldLogger, releaseImage string, releaseImageMirror string, cacheDir string, pullSecret string) (string, error) {
	...
	if releaseImageMirror != "" {
		...
		if err != nil {
			log.Errorf("failed to extract openshift-baremetal-install from mirror release image %s", releaseImageMirror)
			return "", err
		}
	} else {
		path, err = extractFromRelease(log, releaseImage, cacheDir, pullSecret, false)
		if err != nil {
			log.Errorf("failed to extract openshift-baremetal-install from release image %s", releaseImageMirror)
			return "", err
		}
	}
	return path, err
...
func extractFromRelease(log logrus.FieldLogger, releaseImage, cacheDir, pullSecret string, insecure bool) (string, error) {
	...
	if err != nil {
		log.Errorf("error running \"oc adm release extract\" for release %s", releaseImage)
		return "", err
	}
